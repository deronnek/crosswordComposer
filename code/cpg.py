...
import numpy as np
from crosswordComposer import *
# Lots of stuff lifted from
...
class Crossword(object):
    ...
    def order_number_words(self): # orders words and applies numbering system to them
        ...
        # This ordering keeps crosswrd.sty happy, and doesn't affect anything else
        self.current_word_list.sort(key=lambda i, cols=self.cols: (i.row*cols + i.col))
        count, icount = 1, 1
...
nRows = 20
nCols = 20
# 20,000 is max number of loops
a = Crossword(nRows, nCols, '-', 20000, word_list)
# 10 is number of seconds allowed to run
a.compute_crossword(1)

refine = False
# So here I'm going to convert the puzzle into a format my composer can understand, 
# and use it to fill in black squares with dictionary words.

# any element of grid that is equal to a.empty is a black square in the solution
# what I really want to hand my composer is a contiguous chunk of the puzzle, with a 
# number of black squares at it's core, ringed by words.
# or maybe just a contiguous horizontal or vertical segment of black
# the easist thing to do is probably just hand it the complete puzzle, with no black but filled in words,
# then let it run for a while and inspect what it found.  Except that the notions of available spaces are sort of inverted between
# this code and mine.
# I think the best first step is to code the hand-off of the completed puzzle.  My code will have nothing to do, but it will at least
# confirm that the words are being placed in my code's understanding properly.

# Construct black/white matrix

if refine:
	bwMatrix = np.zeros((nRows,nCols), dtype=int)
	for row in range(nRows):
		for col in range(nCols):
			if a.grid[row][col] == a.empty:
				bwMatrix[row][col] = 1

	# Flood fill starting at first detected black square
	first = True
	...
	across, down                   = slotsFromInvalidSquaresMatrix(bwMatrix)
	...
	acrossSeen  = 0
	downSeen    = 0
	for w in a.current_word_list:
		if w.down_across() == 'across':
			acrossFixed.append((w.word, acrossSeen))
			acrossSeen += 1

# To construct the slots I scan down the columns, so we have to re-order the list by column
# to get the down words right.
	wlCopy = duplicate(a.current_word_list)
	wlCopy.sort(key=lambda i: i.col)
	for w in wlCopy:
		if w.down_across() == 'down':
			print w.word, w.number, downSeen
			downFixed.append((w.word, downSeen))
			downSeen += 1

# These do the checking to make sure things line up
	setFixedWords(acrossFixed, acrossWords)
	setFixedWords(downFixed, downWords)
...
print "Couldn't use: "
cwl = [w.word for w in a.current_word_list]
for w in word_list:
    if not (w[0].lower() in cwl):
        print w[0].lower()

with open('generatedCrossword.tex', 'w') as f:
    f.write("\documentclass{article}\n")
    f.write("\usepackage{crosswrd}\n")
    f.write("\\begin{document}\n")
    f.write("\\begin{crossword}{%d}{%s}\n" %(20, 'N'))

    for w in a.current_word_list:
        #f.write("\clue{%d}{%s}{%d}{%d}{%s}{%s}{%d}\n" %(w.number, w.down_across()[0].upper(), w.col, w.row, w.word.upper(), w.clue, len(w.word)))
        f.write("\clue{%d}{%s}{%d}{%d}{%s}{%s}{ }\n" %(w.number, w.down_across()[0].upper(), w.col, w.row, w.word.upper(), w.clue))
         #\clue{1}{A}{1}{1}{SOMERSANDFLEMMER}{This is the clue}{}
    f.write("\end{crossword}\n")
